...
本插件已在以下 Kindle 型号上测试验证通过：

| 设备 | 状态 |
|------|------|
| **Kindle 2024** | ✅ 已验证 |
| **Kindle Paperwhite 11 (KPW5)** | ✅ 已验证 |
...
你会看到多个 `eventX` 文件（X 是数字）。蓝牙控制器**通常是编号最大的那个** —— 它是最后接入的输入设备。

> ⚠️ **注意**：不要配置错误的 event 路径！其他 `eventX` 文件属于触摸屏、物理按键等系统设备，配置错误可能导致异常行为（如拦截触摸屏或电源键事件）。如果不确定，可以断开蓝牙控制器后再次运行 `ls /dev/input`，消失的那个 event 文件就是你的控制器。
...
- **蓝牙开关** — 开启/关闭蓝牙
- **睡眠/唤醒自动开关蓝牙** — 睡眠前关闭蓝牙，唤醒后恢复蓝牙
- **当前设备** — 显示当前用于控制 Kindle 的设备名称
- **已配对蓝牙设备** — 查看已配对设备列表和设备详情
- **重载设备** — 手动重新加载蓝牙输入设备
...
- **手柄未检测到**：先确认蓝牙设备已连接。如需排错，可使用 `ls /dev/input` 看看系统里有没有出现新的 `eventX` 设备。
